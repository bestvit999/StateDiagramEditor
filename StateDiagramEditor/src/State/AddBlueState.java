...
public class AddBlueState implements ControllerState {
	// singleton
	private static AddBlueState instance = null;

	private AddBlueState() {
	}

	public static synchronized AddBlueState getInstance() {
		if (instance == null) {
			return new AddBlueState();
		}
		return instance;
	}
	...
	public ControllerState blackStateBtnClicked(ActionEvent e, Controller c) {
		System.out.println("AddBlueState state : state button clicked, state not change");
	...
	public ControllerState transBtnClicked(ActionEvent e, Controller c) {
		System.out.println("AddBlueState state : transition button clicked, state changing to AddTransition");
		// changing the cursor to crosshair
		c.getView().getCanvas().setCursor(new Cursor(Cursor.CROSSHAIR_CURSOR));
	...
	public ControllerState deleteBtnClicked(ActionEvent e, Controller c) {
		System.out.println("AddBlueState state : delete button clicked, state changing to TransSourceSelected");
	...
	public ControllerState mouseClicked(MouseEvent e, StateDiagram sd, Controller c, DiagramStatus ds) {
		System.out.println("AddBlueState state : mouse Clicked, Create a state at [X=" + e.getX() + ",Y=" + e.getY() + "]");
		...
		if(c.getSelectVersion().equals("UML Version 1.")) {
			...
			//In this case, we set diagramElement implementor to BlueImplementor.		
			state.setImpl(new BlueImplementor());
			sd.add(state);
		...
		// when state diagram add a component, the current stateDiagram would be stored
	...
	public ControllerState mousePressed(MouseEvent e, StateDiagram de, Controller c, DiagramStatus ds) {
		System.out.println("AddBlueState state : mouse pressed, state not change");
	...
	public ControllerState mouseReleased(MouseEvent e, StateDiagram sd, Controller c, DiagramStatus ds) {
		System.out.println("AddBlueState state : mouse Released, state not change");
		// changing cursor to default
		c.getView().getCanvas().setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
		return this;
	}
